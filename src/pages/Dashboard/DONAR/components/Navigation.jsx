        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            ...
            <div className="flex items-center gap-3">
              <div className="bg-#f8a723-50 p-2 rounded-lg">
                <Leaf className="h-6 w-6 text-#f8a723-600" />
              </div>
                ...
                return (
                  <Link
                    ...
                    className={`flex items-center gap-2 px-3 py-2 rounded-md text-sm font-medium transition-all duration-200
                      ${isActive 
                        ? 'bg-#f8a723-50 text-#f8a723-600' 
                        : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900'
                      }`}
                  >
                    <item.icon className={`h-4 w-4 ${isActive ? 'text-#f8a723-600' : 'text-gray-500'}`} />
                    <span>{item.text}</span>
                  </Link>
                );
              })}
            ...
            <div className="px-2 pt-2 pb-3 space-y-1">
              {menuItems.map((item, index) => {
                ...
                return (
                  ...
                  >
                    <item.icon className={`h-5 w-5 ${isActive ? 'text-#f8a723-600' : 'text-gray-500'}`} />
                    <span>{item.text}</span>
                  </Link>
                );
              })}
